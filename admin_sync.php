...
requireAdmin();  // Only administrators can run this

// Log to browser output
...
echo "<h2>Sales Data Synchronization Tool</h2>";
echo "<pre>";

logMessage("Sales data synchronization started at " . date('Y-m-d H:i:s'));
...
try {
    ...
    // 4. Check for test/dummy data that might be causing discrepancies
    logMessage("Checking for sales with zero amount...");
    $stmt = $conn->prepare("SELECT COUNT(*) as count FROM sales WHERE total_amount = 0");
    $stmt->execute();
    $result = $stmt->get_result();
    $row = $result->fetch_assoc();
    $zeroCount = $row['count'];
    ...
    if ($zeroCount > 0) {
        logMessage("Found $zeroCount sales with zero amount");
    } else {
        logMessage("No zero-amount sales found");
    }
    
    // Commit the transaction
    $conn->commit();
    logMessage("Sales data synchronization completed successfully at " . date('Y-m-d H:i:s'));
